...
function lookup_uaddr(b,c){for(var a=0;a<devices.length;a++)if(devices[a].room==b&&devices[a].uaddr==c)return a;logger("lookup_uaddr:: No index in devices "+devices.length+" found for room: "+b+", uaddr: "+c,1);for(a=0;a<devices.length;a++)logger("lookup_addr:: device "+a+", room: "+devices[a].room+", uaddr: "+devices[a].uaddr,1);return-1}
function checkLength(b,c,a,g){return b.val().length>g||b.val().length<a?(b.addClass("ui-state-error"),updateTips("Length of "+c+" must be between "+a+" and "+g+"."),!1):!0}
...
function myAlert(b,c){$('<div style="padding:10px; min-width:350px; max-width:800px; min-height:400px; max-height:500px; overflow:scroll; word-wrap:break-word;">'+b+"</div>").dialog({draggable:!1,modal:!0,resizable:!1,width:"auto",title:c||"Confirm",minHeight:75,dialogClass:"askform",buttons:{OK:function(){$(this).dialog("destroy")}}})}
function myConfirm(b,c,a,g){$('<div style="padding: 10px; max-width: 500px; word-wrap: break-word;">'+b+"</div>").dialog({draggable:!1,modal:!0,resizable:!1,width:"auto",title:g||"Confirm",minHeight:75,dialogClass:"askform",buttons:{OK:function(){"function"==typeof c&&setTimeout(c,50);$(this).dialog("destroy")},Cancel:function(){"function"==typeof a&&setTimeout(a,50);$(this).dialog("destroy")}}})}
...
b,3):logger("action: "+f+", tcnt: "+c+", mes: "+e+", type: "+b,2);break;case "alert":myAlert("Server msg:: "+a.message);break;case "alarm":myAlert("Server msg:: "+a.message);break;case "console":logger("console message received"+a.message,2);myAlert("<br>"+a.message,a.request+" Output");break;case "energy":use_energy||(use_energy=!0,init_menu(s_setting_id));logger("Energy message, kw_act_use: "+a.kw_act_use,1);energy.kw_hi_use=a.kw_hi_use;energy.kw_lo_use=a.kw_lo_use;energy.kw_hi_ret=a.kw_hi_ret;
energy.kw_lo_ret=a.kw_lo_ret;energy.gas_use=a.gas_use;energy.kw_act_use=a.kw_act_use;energy.kw_act_ret=a.kw_act_ret;energy.kw_ph1_use=a.kw_ph1_use;energy.kw_ph2_use=a.kw_ph2_use;energy.kw_ph3_use=a.kw_ph3_use;break;case "graph":logger("rcv:: graph response received, ",1);"function"==typeof display_graph&&display_graph(a.gtype,a.gperiod);break;case "upd":case "gui":var e=a.message;switch(b){case "json":logger("onmessage:: read upd message. Type is: "+b+". Json is not supported yet",1);var k=a.room,
l=a.uaddr,d=a.val,h=lookup_uaddr(k,l);devices[h].val=d;k==s_room_id&&"room"==s_screen?activate_room(s_room_id):["g_grid","g_all","g_room","g_devices"].contains(s_screen)&&update_grid(h);break;case "raw":e=a.message;logger("onmessage:: read upd message. Type is: "+b+", msg: "+e,1);var m=parse_int(e);"!R"==e.substr(0,2)&&(k=m[0],l=m[1],h=lookup_uaddr(k,l),e.search("FdP"),d=m[2],logger("onmessage:: room: "+k+", uaddr: "+l+", val: "+d+", ind: "+h+", scrn: "+s_screen,1),devices[h].val=d,k==s_room_id&&
"room"==s_screen?activate_room(s_room_id):["g_grid","g_all","g_room","g_devices"].contains(s_screen)&&update_grid(h));break;default:logger("onmessage:: read upd message. Unknown type: "+b,1)}0==debug?message("Update: "+devices[h].name+" to value: "+d,0):message("action: "+f+", tcnt: "+c+", mes: "+e+", type: "+b,1);break;case "list_user":logger("list_user message received",1);users=a.message;activate_setting("2b");break;case "load_database":logger("Receiving load_database message",2);lroot=a.response;
rooms=a.response.rooms;devices=a.response.devices;scenes=a.response.scenes;timers=a.response.timers;handsets=a.response.handsets;settings=a.response.settings;brands=a.response.brands;sensors=weather=a.response.sensors;rules=a.response.rules;init();void 0!==typeof Storage&&(localStorage.setObject("rooms",rooms),localStorage.setObject("devices",devices),localStorage.setObject("scenes",scenes),localStorage.setObject("timers",timers),localStorage.setObject("handsets",handsets),localStorage.setObject("settings",
settings),localStorage.setObject("brands",brands),localStorage.setObject("sensors",sensors));message("database received: #rooms: "+rooms.length+", #devices:"+devices.length);break;case "login":if(loginprocess)break;loginprocess=!0;"undefined"!==typeof Storage?(e=localStorage.getItem("uname"),d=localStorage.getItem("pword"),1<=debug&&logger("Support for localstorage, uname: "+e),null===e&&(e=""),null===d&&(d="")):(loger("No local storage in browser",1),message("No local storage in browser"),e="login",
...
sensors[d].humidity+"%")),message(e));break;case "upd_config":logger("rcv:: upd_config message received",1);Object.keys(a.message).forEach(function(b){lroot[b]=a.message[b];switch(b){case "rooms":settings=a.message[b];break;case "devices":devices=a.message[b];break;case "scenes":scenes=a.message[b];break;case "timers":timers=a.message[b];break;case "handsets":handsets=a.message[b];break;case "sensors":sensors=a.message[b];break;case "brands":brands=a.message[b];break;case "settings":settings=a.message[b];
break;default:myAlert("rcv upd_config:: received unknown message key: "+b)}});activate(s_screen);break;case "weather":alert("UNEXPECTED WEATHER MESSAGE RECEIVED");for(d=0;d<weather.length&&(weather[d].address!=a.address||weather[d].channel!=a.channel);d++);d<weather.length&&(weather[d].temperature=a.temperature,weather[d].humidity=a.humidity,weather[d].airpressure=a.airpressure,weather[d].windspeed=a.windspeed,weather[d].winddirection=a.winddirection,weather[d].rainfall=a.rainfall,e="",2<=debug&&
(e+="Weather "+weather[d].name+"@ "+weather[d].location,e+=": temp: "+weather[d].temperature,e+=", humi: "+weather[d].humidity+"%<br>",logger("Weather "+weather[d].name+"@"+weather[d].location+": temp: "+weather[d].temperature+", humi: "+weather[d].humidity+"%")),message(e));break;default:message("Unknown message: action: "+f+", tcnt: "+c+", mes: "+e+", type: "+b)}};logger("Websocket:: readyState: "+w_sock.readyState,1)}

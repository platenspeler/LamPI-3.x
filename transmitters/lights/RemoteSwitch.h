...
class RemoteSwitch {
	public:
		...
		*			The actual number of repeats will be 2^repeats. 3 would be a good start
		*/		
...
class RemoteDimmer {
	public:
		...
		*			The actual number of repeats will be 2^repeats. 3 would be a good start
		*/		
		RemoteDimmer(unsigned short pin, unsigned int periodusec, unsigned short repeats);
		...
		* @return The data suited for use with RemoteSwitch::sendTelegram.
		*/
		unsigned long long encodeTelegram(unsigned short trits[]);
		...
		*
		* Format data:
		* pppppppp|prrrdddd|dddddddd|dddddddd|iiii (36 bit)
		* p = perioud (9 bit unsigned int
		* r = repeats as 2log. Thus, if r = 3, then signal is sent 2^3=8 times
		* d = data
		* i = 4 bits dimlevel
		...
		* @param pin Pin number of the transmitter.
		*/
		static void sendTelegram(unsigned long long data, unsigned short pin);
		...
		* @return true, if the codes are identical (the 20 least significant bits match)
		*/
		static bool isSameCode(unsigned long long encodedTelegram, unsigned long receivedData);
...
/**
* KaKuDimmer simulates a KlikAanKlikUit-remote, but there are many clones.
* If your transmitter has a address dial with the characters A till P, you can try this class.
*/
class KaKuDimmer: RemoteDimmer {
	public:
		/**
		* Constructor
		*
		...
		* @see RemoteSwitch
		*/
		KaKuDimmer(unsigned short pin, unsigned int periodusec=375);
		
		/**
		* Send a dim value a device.
		*
		* @param address	addres (dial switches in remote). Range [A..P] (case sensitive!)
		* @param group	Group to switch. Range: [1..4]
		* @param device	Device to switch. Range: [1..4]
		* @param dim	Range: [1..31] 
		*/
		void sendSignal(char address, unsigned short group, unsigned short device, unsigned short dim);
		...
		* @param on	True, to switch on. False to switch off,
		*/
		void sendSignal(char address, unsigned short device, unsigned short dim);
		
		/**
		* @see RemoteSwitch::getTelegram
		*/
		unsigned long long getTelegram(char address, unsigned short group, unsigned short device, unsigned short dim);
		
		/**
		* @see RemoteSwitch::getTelegram
		*/
		unsigned long long getTelegram(char address, unsigned short device, unsigned short dim);
};

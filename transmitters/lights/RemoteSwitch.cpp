 ...
 * Ported to RaspberryPI by M. Westenberg on Dec 2013
 *
...
RemoteDimmer::RemoteDimmer(unsigned short pin, unsigned int periodusec, unsigned short repeats) {
	_pin=pin;
	_periodusec=periodusec;
	_repeats=repeats;
...
unsigned long long RemoteDimmer::encodeTelegram(unsigned short trits[]) {
	unsigned long data = 0;
...
void RemoteDimmer::sendTelegram(unsigned short trits[]) {
	sendTelegram(encodeTelegram(trits),_pin);	
}
...
* d = data
*/
void RemoteDimmer::sendTelegram(unsigned long long data, unsigned short pin) {
	unsigned int periodusec = (unsigned long)data >> 23;
	unsigned short repeats = 5 << (((unsigned long)data >> 20) & 7); // 7 = B111
	data = data & 0xfffff; //truncate to 20 bit
...
bool RemoteDimmer::isSameCode(unsigned long long encodedTelegram, unsigned long receivedData) {
	return (receivedData==(encodedTelegram & 0xFFFFF)); //Compare the 20 LSB's
}
...
KaKuDimmer::KaKuDimmer(unsigned short pin, unsigned int periodusec) : RemoteDimmer(pin,periodusec,3) {
	//Call contructor
}
...
/* Address = 
   ...
   dim = 1..31
*/
void KaKuDimmer::sendSignal(char address, unsigned short device, unsigned short dim) 
{
	sendTelegram(getTelegram(address, device, dim), _pin);
}

/*
  return value is a 36-bits message (not 32 bits) must be long long
*/
unsigned long long KaKuDimmer::getTelegram(char address, unsigned short device, unsigned short dim) 
{
	unsigned short trits[12];
	
	address-=65;			// Address is "A"-"P", char codes addr = code(addr) - code("A");

	device-=1;			// devices range from 1-32, so this maps back to a range 0-31
	...
	//switch on or off  XXX MUST BE CHANGED FOR DIMMER
	trits[11]=(dim?2:0);

	// returning 36 -bits message for dimmer	
	return encodeTelegram(trits);
}

// ---------------------------------
// GROUP COMMAND IS FOR NEW REMOTES. 
// For Raspberry we probably do NOT need the group variant
//
void KaKuDimmer::sendSignal(char address, unsigned short group, unsigned short device, unsigned short dim) 
{
	sendTelegram(getTelegram(address, group, dim), _pin);
}

//
//
unsigned long long KaKuDimmer::getTelegram(char address, unsigned short group, unsigned short device, unsigned short dim) 
{
	unsigned short trits[12], i;

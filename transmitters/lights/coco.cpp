...
{
    int pin_out = 15; // Pin out using wiringPi pin numbering scheme 
			// (15 = TxD / BCM GPIO 14, 
	...
	if ( !dimmer ) {
		...
		kaKuSwitch.sendSignal(address, device, state);
    		...
    		std::cout << "debug: " << argv[0] << " A 4 statestr " << std::endl;
		std::cout << "size long long: " << sizeof(beer)  << std::endl;
		
		state = true;
		KaKuDimmer kaKuDimmer(pin_out);
		kaKuDimmer.sendSignal(address, device, dim);
	}
}

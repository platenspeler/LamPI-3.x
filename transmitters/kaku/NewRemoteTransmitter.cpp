...
NewRemoteTransmitter::NewRemoteTransmitter(unsigned long address, unsigned char pin, unsigned int periodusec, unsigned char repeats) {
        _address = address;
        _pin = pin;
...
void NewRemoteTransmitter::_sendBit(bool isBitOne) {
        if (isBitOne) {
                ...
                delayMicroseconds(_periodusec);
                digitalWrite(_pin, LOW);
                delayMicroseconds(_periodusec * 5);			// was 5
                digitalWrite(_pin, HIGH);
                delayMicroseconds(_periodusec);
                digitalWrite(_pin, LOW);

...
struct NewRemoteCode {
        ...
        unsigned char dimLevel;         // Dim level [0..15] iff switchType == 2
};
...
* as well as the signal sent by the RemoteSwitch class. When a correct signal is received,
...
class NewRemoteReceiver {
        public:
			...
			*
			* @param interrupt The interrupt as is used by Arduino's attachInterrupt function. 
			...
			*/
			static void init(char interrupt, unsigned char minRepeats, NewRemoteReceiverCallBack callback);
			...
			*/
			...
			/**
			* Tells wether a signal is being received. If a compatible signal is detected within the time out, 
			*		isReceiving returns true.
			* Since it makes no sense to transmit while another transmitter is active, it's best to wait 
			*		for isReceiving() to false.
			* By default it waits for 150ms, in which a (relative slow) KaKu signal can be broadcasted three times.
			*
			* Note: isReceiving() depends on interrupts enabled. Thus, when disabled()'ed, or when 
			*		interrupts are disabled (as is the case in the callback), isReceiving() will not work properly.
			*
			* @param waitMillis number of milliseconds to monitor for signal.
			* @return boolean If after waitMillis no signal was being processed, returns false. 
			*		If before expiration a signal was being processed, returns true.
			*/
        ...
        private:
			...
			static char _interrupt;						// Radio input interrupt
			...
			static bool _inCallback;                    // When true, the callback function is being executed; 
														// prevents re-entrance.
			static bool _enabled;						// If true, monitoring and decoding is enabled. 

var debug=1,poll_interval=6E3,log_interval=12E4,alarm_interval=2E3,timer_interval=3E4,webPort=8080,sockPort=5E3,udpPort=5001,tcpPort=5002,tcnt=0,zroot={},devices={},clients=[],loops=[],config={},lampi_admin=[],homeDir="/home/pi",logDir=homeDir+"/log",rrdDir=homeDir+"/rrd",wwwDir=homeDir+"/www",par=require("./config/params"),mysql=require("mysql");console.log("mySQL loaded");var fs=require("fs");console.log("fs loaded");var strip=require("strip-json-comments");console.log("strip-json-comments loaded");
...
logger(c,2);b.send(c);start_loops()})});app.all("/sensors",function(a,b,d){console.log("Accessing the sensors section ...");b.send("sensors")});app.all("/config",function(a,b,d){console.log("Printing configuration ...");b.send(printConfig())});app.use("/alarm",alarmRouter);function readConfig(){var a=fs.readFileSync(homeDir+"/config/database.cfg","utf8"),a=JSON.parse(strip(a));3<=debug&&(logger("readConfig:: config read: ",3),console.log(a));return a}
...
function createDbase(a){async.series([function(a){queryDbase("CREATE TABLE IF NOT EXISTS users(id INT, descr CHAR(128), name CHAR(20), login CHAR(20), passw CHAR(32), class INT )",function(d,c){var e=[];d?a(d,null):queryDbase("SELECT * FROM users",function(c,d,h){if(c)a(c,null);else if(0==d.length){for(d=0;d<config.users.length;d++)insertDb("users",config.users[d],function(a){e.push("u")});a(c,"uers made, sizeof users is:  "+config.users.length)}else logger("createDbase:: WARNING: There are already users in the user table"),
a(c,"existing users")})})},function(a){queryDbase("DROP TABLE IF EXISTS rooms",function(d,c){queryDbase("CREATE TABLE rooms(id INT, descr CHAR(128), name CHAR(20) )",function(d,c){a(d,"rooms made")})})},function(a){queryDbase("DROP TABLE IF EXISTS devices",function(d,c){queryDbase("CREATE TABLE devices(id CHAR(3), descr CHAR(128), uaddr CHAR(3), gaddr CHAR(12), room CHAR(12), name CHAR(20), type CHAR(12), val INT, lastval INT, brand CHAR(20) )",function(d,c){a(d,"devices made")})})},function(a){queryDbase("DROP TABLE IF EXISTS scenes",
function(d,c){queryDbase("CREATE TABLE scenes(id INT, descr CHAR(128), val INT, name CHAR(20), seq CHAR(255) )",function(d,c){a(d,"scenes made")})})},function(a){queryDbase("DROP TABLE IF EXISTS timers",function(d,c){queryDbase("CREATE TABLE timers(id INT, descr CHAR(128), name CHAR(20), scene CHAR(20), tstart CHAR(20), startd CHAR(20), endd CHAR(20), days CHAR(20), months CHAR(20), skip INT )",function(d,c){a(d,"timers made")})})},function(a){queryDbase("DROP TABLE IF EXISTS handsets",function(d,
c){queryDbase("CREATE TABLE handsets(id INT, descr CHAR(128), name CHAR(20), brand CHAR(20), addr CHAR(20), unit INT, val INT, type CHAR(20), scene CHAR(255) )",function(d,c){a(null,"handsets made")})})},function(a){queryDbase("DROP TABLE IF EXISTS settings",function(d,c){queryDbase("CREATE TABLE settings(id INT, descr CHAR(128), val CHAR(128), name CHAR(20) )",function(d,c){a(null,"settings made")})})},function(a){queryDbase("DROP TABLE IF EXISTS controllers",function(d,c){queryDbase("CREATE TABLE controllers(id INT, descr CHAR(128), name CHAR(20), fname CHAR(128) )",
function(d,c){a(null,"controllers made")})})},function(a){queryDbase("DROP TABLE IF EXISTS brands",function(d,c){queryDbase("CREATE TABLE brands(id INT, descr CHAR(128), name CHAR(20), fname CHAR(128) )",function(d,c){a(null,"brands made")})})},function(a){queryDbase("DROP TABLE IF EXISTS sensors",function(d,c){queryDbase("CREATE TABLE sensors(id INT, descr CHAR(128), name CHAR(20), location CHAR(20), brand CHAR(20), address CHAR(20), channel CHAR(8), type CHAR(32), caps CHAR(64), sensor CHAR(255) )",
function(d,c){a(null,"sensors made")})})},function(a){queryDbase("DROP TABLE IF EXISTS rules",function(d,c){queryDbase("CREATE TABLE rules(id INT, descr CHAR(128), name CHAR(20), active CHAR(1), jrule TEXT(65000), brule TEXT(65000) )",function(d,c){a(null,"rules made")})})},function(a){var d=[];logger("createDb starting for devices, #devices: "+config.devices.length);for(var c=0;c<config.devices.length;c++)insertDb("devices",config.devices[c],function(a,b){d.push("d")});a(null,"devices: "+d)},function(a){var d=
[];logger("createDb starting for rooms, #rooms: "+config.rooms.length);for(var c=0;c<config.rooms.length;c++)insertDb("rooms",config.rooms[c],function(a){d.push("r")});a(null,"fill rooms"+config.rooms.length)},function(a){for(var d=[],c=0;c<config.scenes.length;c++)insertDb("scenes",config.scenes[c],function(a){d.push("s")});a(null,"scenes: "+d)},function(a){for(var d=[],c=0;c<config.timers.length;c++)insertDb("timers",config.timers[c],function(a){d.push("t")});a(null,"timers: "+d)},function(a){for(var d=
[],c=0;c<config.handsets.length;c++)insertDb("handsets",config.handsets[c],function(a){d.push("h")});a(null,"handsets: "+d)},function(a){for(var d=[],c=0;c<config.settings.length;c++)insertDb("settings",config.settings[c],function(a){d.push("x")});a(null,"settings: "+d)},function(a){for(var d=[],c=0;c<config.controllers.length;c++)insertDb("controllers",config.controllers[c],function(a){d.push("c")});a(null,"controllers: "+d)},function(a){for(var d=[],c=0;c<config.brands.length;c++)insertDb("brands",
...
b,function(a){logger("add_sensor finished OK "+a,1)});config.sensors.push(b);break;case "delete_sensor":deleteDb("sensors",b,function(a){logger("delete_sensor finished OK "+a,1)});delFromArray(config.sensors,b);break;case "add_rule":config.rules.push(b);a={name:b.name,id:b.id,descr:b.descr,jrule:JSON.stringify(b.jrule),brule:JSON.stringify(b.brule)};insertDb("rules",a,function(a){logger("add_rule finished OK "+a,1)});break;case "store_rule":updInArray(config.rules,b);a={name:b.name,id:b.id,active:b.active,
descr:b.descr,jrule:JSON.stringify(b.jrule),brule:JSON.stringify(b.brule)};updateDb("rules",a,function(a){logger("store_rule finished OK "+a,1)});break;case "delete_rule":deleteDb("rules",b,function(a){logger("delete_rule finished OK "+a,1)});delFromArray(config.rules,b);break;case "store_setting":"debug"==b.name&&(debug=Number(b.val));"alarm"==b.name&&(3<=d.trusted?woonveilig.wvSet(b.val,function(a,d){logger("Alarm set to: "+d)},1):(c={tcnt:tcnt++,type:"raw",action:"login",message:"Alarm can only be set with a user that has a higher trustlevel"},
d.send(JSON.stringify(c),function(a){void 0!==a&&(logger("store_setting:: ERROR sending login request "+a,1),logger("store_setting:: Socket: "+d.name+", type: "+d.type,1))}),a=iArray(config.settings,b),b.val=config.settings[a].val,logger("store_setting:: trust of "+d.name+" too low: "+d.trusted+"to change "+config.settings[a].name,1)));updateDb("settings",b,function(a){logger("store_settings finished OK "+a,1);updInArray(config.settings,b);a={};a.settings=config.settings;a={tcnt:tcnt++,type:"json",
action:"upd_config",message:a};d.send(JSON.stringify(a),function(a){void 0!==a&&(logger("store_setting:: ERROR sending settings "+a,1),logger("store_setting:: Socket: "+d.name+", type: "+d.type,1))})});break;case "add_device":insertDb("devices",b,function(a){logger("add_device finished OK",1)});config.devices.push(b);break;case "delete_device":delDevDb("devices",b,function(a){logger("delete_device finished OK",1)});a=idDevice(b.room,b.id);-1!=a?config.devices.splice(a,1):logger("dbaseHandler:: delete_device failed for index: "+
a+", room: "+b.room+", id: "+b.id);break;case "store_device":updDevDb("devices",b,function(a){logger("store_device finished OK",1)});a=idDevice(b.room,b.id);-1!=a?config.devices[a]=b:logger("dbaseHandler:: store_device failed for index: "+a);break;default:logger("dbaseHandler:: Unknown command: "+a,1)}}
...
Math.floor(1E3*Number(a.kw_act_ret));c+=":"+Math.floor(1E3*Number(a.kw_ph1_use));c+=":"+Math.floor(1E3*Number(a.kw_ph2_use));c+=":"+Math.floor(1E3*Number(a.kw_ph3_use));d="rrdtool update "+d+" N"+c;logger("energyHandler:: execStr: "+d,2);exec(d,function(a,d,c){null===a?logger("energyHandler:: stdout: "+d+"; stderr: "+c,2):logger("energyHandler:: ERROR: "+a+"; stderr: "+c)});broadcast(JSON.stringify(a),b,"raw")}
function graphHandler(a,b){var d=a.gcmd,c=a.gtype,e=a.gperiod,f=a.gsensors,g,h,k,l,m="e350.rrd",n="ff0000 111111 00ff00 0000ff ff00ff 666666 00ffff ff3399 ffff00".split(" ");logger("graphHandler:: Starting for "+d+":"+c+":"+e+":"+f);void 0==f&&(logger("graphHandler:: No sensors defined",1),f=[]);switch(d){case "energy":switch(c){case "E_GAS":k=sensorType="gas_use";l="M3";break;case "E_ACT":sensorType="pwr act";k="pwr_act";l="kWhr";break;case "E_USE":sensorType="pwr use";k="pwr_use";l="kWhr";break;
case "E_PHA":sensorType="phase use",k="pwr_pha",l="kWhr"}break;case "weather":case "sensors":switch(c){case "T":sensorType="temperature";k="all_temp";l="C";break;case "H":sensorType="humidity";k="all_humi";l=" %%";break;case "P":sensorType="airpressure",k="all_press",l="hPa"}}switch(e){case "1h":g="";break;case "1d":g="";break;case "1w":g=":step=3600";break;case "1m":g=":step=8640";break;case "1y":g=":step=21300";break;default:logger("graphHandler:: ERROR unknown period: "+e,1)}for(var p=c="",t="",
q=0;q<f.length;q++)h=q+1==f.length?"\\n":"","energy"==d?(m="e350.rrd",c+="DEF:t"+(q+1)+"=/home/pi/rrd/db/"+m+":"+f[q]+":AVERAGE"+g+" "):(m=f[q]+".rrd",c+="DEF:t"+(q+1)+"=/home/pi/rrd/db/"+m+":"+sensorType+":AVERAGE "),p+="LINE2:t"+(q+1)+"#"+n[q%n.length]+':"'+f[q]+h+'" ',t+="GPRINT:t"+(q+1)+':LAST:"'+f[q]+" %3.0lf "+l+h+'" ';d="/usr/bin/rrdtool graph /home/pi/www/graphs/"+k+"_"+e+".png"+(" -s N-"+e+' -a PNG -E --title="'+sensorType+' readings" ');d+='--vertical-label "'+sensorType+'" --width 750 --height 500 ';
d+=c;d+=p;d+=t;logger("graphHandler:: \n"+d,2);exec(d,function(d,c,e){null===d?(logger("graphHandler:: ok, stdout: "+c+"; stderr: "+e,2),broadcast(JSON.stringify(a),b,"raw")):logger("graphHandler:: ERROR: "+d+"; stderr: "+e)})}
...
config.handsets[e],e.type){case "scene":queueScene(e.scene);break;default:logger("icsHandler:: handset type not recognized: "+e.type,1)}else logger("icsHandler:: Handset "+h+" not found",1);break;case "!Q":logger("icsHandler:: All Off Q command",1);break;default:logger("icsHandler:: Unknown command ics code: <"+d+">",1)}}
...
":"+Number(a.airpressure):"";f+=void 0!==a.altitude?":"+Number(a.altitude):"";f+=void 0!==a.windspeed?":"+Number(a.windspeed):"";f+=void 0!==a.winddirection?":"+Number(a.winddirection):"";f+=void 0!==a.rainfall?":"+Number(a.rainfall):"";f+=void 0!==a.luminescense?":"+Number(a.luminescense):"";d="rrdtool update "+e+" N"+f;logger("sensorHandler:: execStr: "+d,2);exec(d,function(d,c,b){null===d?logger("sensorHandler:: stdout: "+c+"; stderr: "+b,2):(logger("sensorHandler:: ERROR: for rrd str: "+f+"\n Rrd Error: "+
d+"; stderr: "+b,0),console.log("sensorHandler:: string: ",a))});broadcast(JSON.stringify(a),b,"raw")}}
...
function queueHandler(){for(var a=queue.qpop();null!=a&&0<a.length;){2<=debug&&console.log("queueHandler:: pop task: ",a);for(var b=0;b<a.length;b++){var d;1<=debug&&console.log("\t\tqueueHandler:: processing task: ",a[b]);switch(a[b].name){case "gui":d=a[b].seq.split(",");break;case "scene":logger("queueHandler:: scene found",1);default:logger("queueHandler:: executing: "+a[b].name,1),d=findScene(a[b].name),d=config.scenes[d].seq.split(",")}for(var c=0;c<d.length;c+=2)var e={tcnt:""+tcnt++,type:"raw",
action:"gui",cmd:"",gaddr:"",uaddr:"",val:"",message:d[c]};socketHandler(JSON.stringify(e))}a=queue.qpop()}2<=debug&&queue.qprint()}
function ruleHandler(){for(var a=0;a<config.rules.length;a++)if("Y"==config.rules[a].active){logger("ruleHandler:: rule "+config.rules[a].name+" is active",2);try{logger("ruleHandler:: eval "+config.rules[a].name,1),logger("ruleHandler:: "+config.rules[a].jrule,2),eval(config.rules[a].jrule)}catch(b){logger("ruleHandler:: Error evaluating rule "+config.rules[a].name+", error: "+b,1)}}else logger("ruleHandler:: rule "+config.rules[a].name+" not active",3);logger("ruleHandler:: Ended",2)}
logger("MAIN part started",1);async.series([function(a){connectDbase(a)},function(a){zwave_init(function(b,d){a(null,d)})},function(a){loadDbase(a)}],function(a,b){main(a,b)});function start_loops(){loops=[];logger("All Init functions done",1);alarm_loop();timer_loop();poll_loop();zwave_loop()}
function main(a,b){1<=debug&&console.log("Return values: \n",b);userver.bind(udpPort);logger("Starting Loops");start_loops();logger("Starting Static Webserver");app.use(serveStatic(__dirname+"/www"));app.listen(webPort)}
...
e[k].tstart.split(":"),m=l[0],n=l[1],l=e[k].startd.split("/"),p=l[0],t=l[1],q=Number(l[2])+2E3,l=e[k].endd.split("/"),w=l[0],A=l[1],l=Number(l[2])+2E3,r,v,x,y,z,u;x=a.getDate();y=a.getMonth();z=a.getFullYear();switch(m){case "96":u=1800*-Number(n);r=h.sunrise.getHours();v=h.sunrise.getMinutes();break;case "97":u=1800*Number(n);r=h.sunrise.getHours();v=h.sunrise.getMinutes();break;case "98":u=1800*-Number(n);r=h.sunset.getHours();v=h.sunset.getMinutes();break;case "99":u=1800*Number(n);r=h.sunset.getHours();
v=h.sunset.getMinutes();break;default:u=0,r=m,v=n}r=Math.floor((new Date(z,y,x,r,v,0,0)).getTime()/1E3)+u;logger("Timer correction is: "+u,2);logger("timer_loop:: run   date: "+r,2);logger("timer_loop:: now   date: "+d,2);0<r-d&&logger("timer_loop:: name: "+e[k].name+", runnable in: "+(r-d)+" secs",1);if(d<r)logger("Timer not yet started ",2);else if(w=Math.floor((new Date(l,A-1,w,0,0,0,0)).getTime()/1E3),logger("timer_loop:: end date:   "+w,2),d>w)logger("Timer enddate reached: "+w,1);else if(m=
Math.floor((new Date(q,t-1,p,m,n,0,0)).getTime()/1E3),logger("timer_loop:: start date: "+m,2),d<m)logger("Timer before start date",1);else if(d-r>timer_interval/500+1)logger("Timer already started some time ago",2);else if(d-r>timer_interval/1E3)logger("Timer already started",2);else if("x"==e[k].months[Number(a.getMonth())-1])logger("Timer, this is a blackout month",1);else if("x"==e[k].days[Number(a.getDay())-1])logger("Timer, this is a blackout day",1);else for(logger("timer_loop:: Starting timer name: "+
e[k].name,1),m=0;m<config.scenes.length;m++)if(g[m].name==e[k].scene)for(n=g[m].seq.split(","),p=0;p<n.length;p+=2)logger("scene item: "+p/2+", val: "+n[p]+", time: "+n[p+1],1),t=n[p+1].split(":"),t=+(3600*t[0])+60*t[1]+t[2],logger("qinsert:: ticks: "+t+", name: "+g[m].name+", seq: "+n[p],1),queue.qinsert({ticks:Number(t)+getTicks(),name:"gui",seq:n[p]})}}))})},timer_interval);loops.push(a)}
